...
class Trainer:
    ...
    def _training_step(
        ...
        if global_step >= self.args.kl_start_step and (
                self.args.att_type == 'soft_weibull' or self.args.att_type == 'soft_lognormal') and self.args.att_kl != 0:
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            label_tensors = torch.cat((label_tensors, inputs["labels"].detach().cpu()), 0)
            score_tensors = torch.cat((score_tensors, logits.detach().cpu()), 0)
            # print(inputs["labels"])
            # print(logits)
        ...
        ece = ECELoss(n_bins=20)(score_tensors, label_tensors)
        print('ece', ece)
        # logger.info("  ECE:.2f", ece)
